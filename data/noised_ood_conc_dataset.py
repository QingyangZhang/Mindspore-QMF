...
def fixed_noise(nc, width, height, index):
...
class noisedConcDataset:
    ...
    def __init__(self, cfg, data_dir=None, rgb_transform=None, depth_transform=None, normal_transform = None, labeled=True, rgb_threshold=0.5, depth_threshold=0.5):
        self.cfg = cfg
    ...
    def __getitem__(self, index):
        ...
        #AB_conc_3 = random_noise(3, AB_conc_1.size[1], AB_conc_1.size[0])
        ...
        if w2 > self.cfg.FINE_SIZE:
            ...
            #A_3 = AB_conc_3.crop((0, 0, w2, h)).resize((self.cfg.LOAD_SIZE, self.cfg.LOAD_SIZE), Image.BICUBIC)
            #B_3 = AB_conc_3.crop((w2, 0, w, h)).resize((self.cfg.LOAD_SIZE, self.cfg.LOAD_SIZE), Image.BICUBIC)             
        else:
        ...
        if self.labeled:
            pa = np.random.uniform()>0.2
            pb = np.random.uniform()>0.2
